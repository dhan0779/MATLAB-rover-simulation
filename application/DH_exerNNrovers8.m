...
switch repeet
    ...
    case {0, 2, 3}
        [pp, pp2, cent_mat, ang_vec, feeld, jmp ] = CreateMdeLs6B(bool1,bool2,bool3,bool4, reso, radi, NN, energy ) ;
...
for kk = 1:kk_max
    ...
    kdist = 3.5 - kdist0*exp(-kk/100) ; %asymptote from 1.5 to 3 tau 100 kk
    ...
    figure(1)
    if repeet ~= 3
        PrintPlotRR3(bool1,bool2,bool3,bool4,radi,pp,ang_vec, reso, pp2, cent_mat, feeld, kk, NN,energy )
    end
    ...
    if kk > 4
        ...
        [ inact, row_kill, pp2, cent_mat, jmp ] = Inactivate_test4(bool1,bool2,bool3,bool4, radi,pp,ang_vec,pp2, cent_mat, kdist, reso, jmp, kk, repeet, R1, R2, die_flg,energy ) ;
        if floor(row_kill) == row_kill && row_kill > 0
            energy(int8(row_kill)) = energy(int8(row_kill)) + 20;

        end
        if floor(inact) == inact && inact > 0 && inact == 4
            jmp(4) = 0; 
            energy(4) = -1;
            bool4 = 1;
        end
        if floor(inact) == inact && inact > 0 && inact == 3
            jmp(3) = 0; 
            energy(3) = -1;
            bool3 = 1;
        end
        if floor(inact) == inact && inact > 0 && inact == 2
            jmp(2) = 0; 
            energy(2) = -1;
            bool2 = 1;
        end
        if floor(inact) == inact && inact > 0 && inact == 1
            jmp(4) = 0; 
            energy(1) = -1;
            bool1 = 1;
        end
