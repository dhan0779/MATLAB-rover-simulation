function [ KwadT, KwadB, topp, ppA, ppB, cent1, cent2, ang1, ang2 ] = Quad_test_collision2( ppA, ppB, cent1, cent2, ang1, ang2, jmp, reso, radi, feeld )
...
[ ppA, cent1, ang1 ] = Rot_sub_Cent2(ppA, cent1, ang1, alphT) ;
[ ppB, cent2, ang2 ] = Rot_sub_Cent2(ppB, cent2, ang2, alphB) ;
    
radi_sq = (2*radi)^2 ; % test for continued overlap below 
cent_dist_sq = sumsqr(cent1 - cent2);
while cent_dist_sq < radi_sq
    mid_est = feeld/2 ; % max( [ppA ppB]'/2 ) ; 
    cent_dist_midA = sqrt( sumsqr( cent1 - mid_est ) );
    cent_dist_midB = sqrt( sumsqr( cent2 - mid_est ) );
    [ YY, ndx] = min( [ cent_dist_midA cent_dist_midB ] );  
    switch ndx
        case 1
            ang2mid = atan2(cent1(2) - mid_est(2), cent1(1) - mid_est(1) ) + pi; % dir it seems 
            ppA(1, :) = ppA(1, :) + cos(ang2mid)*1 * ones(1, reso) ;  % go to the middle
            ppA(2, :) = ppA(2, :) + sin(ang2mid)*1  * ones(1, reso) ;
            cent1 = [ cent1(1)+cos(ang2mid)*1  cent1(2)+sin(ang2mid)*1  ];  % jmp out in while
            
        case 2
            ang2mid = atan2(cent2(2) - mid_est(2), cent2(1) - mid_est(1) ) + pi ;
            ppB(1, :) = ppB(1, :) + cos(ang2mid)*1 * ones(1, reso) ;  % go to the middle
            ppB(2, :) = ppB(2, :) + sin(ang2mid)*1  * ones(1, reso) ;
            cent2 = [ cent2(1)+cos(ang2mid)*1  cent2(2)+sin(ang2mid)*1  ]; 
    end
    cent_dist_sq = sumsqr(cent1 - cent2);
end
