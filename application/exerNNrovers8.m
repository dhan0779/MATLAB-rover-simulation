function [ feeld, kk, scor_mat, FF ] = exerNNrovers8( repeet, NoRnd, radi, reso, kk_max )
% rev 8 incorp matlab movie FF feature 8.1.18
% repeet 3 to supress plot
% replay example:  [ feeld, kk, scor_mat, FF ] = exerNNrovers8( 0, 0, 2.25, 17, 222 ) ;
...
%radi = 2.25;
tailT = 7;
NN = 4; % number of rovers in field 
skip_tail = 0;
done_flg = 0;
scor_mat(1, : ) = [ 0 0 0  ] ; % init scoring
s2 = 1;
Only2 = 0;
die_flg = 0;
...
save C:/MatlabR12/DahTah/WGdata18/BlkHole40 Only2
% then only when only 2 left does Only2 change to 1...

switch repeet
    case 1
        load C:/MatlabR12/DahTah/WGdata18/Create_sve
        %load C:/MatlabR12/DahTah/WGdata18/Create_sve74
        %load  C:/MatlabR12/DahTah/WGdata18/Create_sve85
    case {0, 2, 3}
...
save C:/MatlabR12/DahTah/WGdata18/ScoreTable scor_mat s2 done_flg
...
for kk = 1:kk_max
    ...
    switch NoRnd
        case 0
            ...
            %save C:/MatlabR12/DahTah/WGdata18/alph1_sve  alph1 alph2
            save C:/MatlabR12/DahTah/WGdata18/RRU  RandRoundUp
        case 1
            load C:/MatlabR12/DahTah/WGdata18/alph1_sve
        case { 2, 3 }
            load C:/MatlabR12/DahTah/WGdata18/RRU
    ...
    load C:/MatlabR12/DahTah/WGdata18/BlkHole40
    if Only2 == 1
    ...
    if NoRnd == 3
        load C:/MatlabR12/DahTah/WGdata18/PP_check
        pp2(:, :, 1) = pp_arry(:, :, 1, kk) ;
        pp2(:, :, 2) = pp_arry(:, :, 2, kk)  ;
        pp2(:, :, 3) = pp_arry(:, :, 3, kk)  ;
        pp2(:, :, 4) = pp_arry(:, :, 4, kk)  ;
    ...
    if repeet ~= 2 || repeet ~= 3
        drawnow
        FF(kk) = getframe;
        save C:/MatlabR12/DahTah/WGdata18/FFnow  FF
    end
    ...
    if NoRnd == 0
        ...
        pp_arry(:, :, 3, kk) = pp2(:, :, 3) ;
        pp_arry(:, :, 4, kk) = pp2(:, :, 4) ;
        save C:/MatlabR12/DahTah/WGdata18/PP_check   pp_arry cen_arry ang_arry jmp_arry
    end
    if  repeet ~= 3
        pause(psr)
    end
    ...
    if kk > 4
        save C:/MatlabR12/DahTah/WGdata18/cent_sve cent_mat  feeld  NN
        [ inact, row_kill, pp2, cent_mat, jmp ] = Inactivate_test4( pp2, cent_mat, kdist, reso, jmp, kk, repeet, R1, R2, die_flg ) ;
        load  C:/MatlabR12/DahTah/WGdata18/ScoreTable
        if done_flg == 1
            return
        end
    end
