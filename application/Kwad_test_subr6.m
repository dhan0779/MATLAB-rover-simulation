function [KwadT, KwadB,  pp2, cent_mat, ang_vec, die_flg ] = Kwad_test_subr5( pp2, cent_mat, ang_vec, R1, R2, jmp, radi, reso, kk, NoRnd )
...
%load H:/Documents/MATLAB/DahTah/WGdata18/cent_dist_list

ang = ang_vec;
det_flg = 1;
die_flg = 0; % the own-goal type loss 
%topp = R1;
cnt = 1; 
while det_flg == 1
   ...
   switch NoRnd 
    ...
    switch NoRnd
       case 0
          ...
          save H:/Documents/MATLAB/DahTah/WGdata18/RRU RandRoundUp
          %H:/Documents/MATLAB/DahTah/WGdata18/alphCW_sve alphCW alphCCW
       case { 2, 3 }
          load H:/Documents/MATLAB/DahTah/WGdata18/RRU
          % load  H:/Documents/MATLAB/DahTah/WGdata18/alphCW_sve
...
if pp2(1, (reso-1)/2+1, R1) > 40 || pp2(1, (reso-1)/2+1, R1) < 0 % (reso-1)/2+1 is the V switch 
    die_flg = R1; % horizontal: Left, Right 
end

if pp2(2, (reso-1)/2+1, R1) > 40 || pp2(2, (reso-1)/2+1, R1) < 0 % (reso-1)/2+1 is the V switch 
    die_flg = R1; %vertical, up down 
end

if pp2(1, (reso-1)/2+1, R2) > 40 || pp2(1, (reso-1)/2+1, R2) < 0 % (reso-1)/2+1 is the V switch 
    die_flg = R2; % horizontal: Left, Right 
end

if pp2(2, (reso-1)/2+1, R2) > 40 || pp2(2, (reso-1)/2+1, R2) < 0 % reso/2+1 is the V switch 
    die_flg = R2; %vertical, up down 
end

ang_vec = ang; 

if die_flg > 0 
    disp('Edge kill in Kwad6 func') 
     %keyboard 
end
